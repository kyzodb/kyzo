...
mod tests {
    ...
    fn offsets_and_values_starts_at_zero_and_is_monotone() -> Result<()> {
        ...
        let mut off_i32 = Vec::with_capacity(offsets.len() / 4);
        for c in offsets.chunks_exact(4) {
            let bytes: [u8; 4] = c
                .try_into()
                .map_err(|_| miette!("offset chunk must be exactly 4 bytes"))?;
            off_i32.push(i32::from_le_bytes(bytes));
        }
