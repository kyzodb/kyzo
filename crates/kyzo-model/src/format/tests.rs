...
//!
//! Expr canonical serde golden lives beside the Expr authority in
//! `program/expr.rs` (one seat).
...
fn corpus() -> Result<Vec<DataValue>> {
    let mut c = vec![
        ...
        DataValue::Num(Num::int((1 << 53) + 1)),
        DataValue::Num(Num::float(9_007_199_254_740_992.0)),
        ...
        DataValue::Validity(ValiditySlot::from_stored(ValidityTs::of_micros(0), true)),
        DataValue::Validity(ValiditySlot::from_stored(ValidityTs::of_micros(1), false)),
...
// Expr canonical round-trip golden lives beside the Expr authority in
// `program/expr.rs` — one seat (copy_detector).
