...
mod tests {
    ...
    fn intern_num(arena: &mut Arena, n: i64) -> Result<StampedCode> {
        ...
        match Value::mint(&cb, arena).into_diagnostic()?.stamp() {
    ...
    fn sealed_fast_lane_agrees_with_byte_order() -> Result<()> {
        ...
        expect.sort_by(|&i, &j| {
            adm.resolve(match usize::try_from(i) { Ok(n) => n, Err(_) => 0 })
                .into_diagnostic()?
                .cmp(adm.resolve(match usize::try_from(j) { Ok(n) => n, Err(_) => 0 }).into_diagnostic()?)
        });
        ...
        let by_perm: Vec<&[u8]> = perm
            .iter()
            .map(|&i| adm.resolve(match usize::try_from(i) { Ok(n) => n, Err(_) => 0 }).into_diagnostic()?)
            .collect();
    ...
    fn gather_preserves_values_and_sortedness_and_readmits() -> Result<()> {
        ...
        let before: Vec<Vec<u8>> = {
            ...
            let adm = col.admit(&f).into_diagnostic()?;
            (0..adm.len())
                .map(|i| adm.resolve(i).into_diagnostic()?.to_vec())
                .collect()
        };
        let remap = arena.seal().into_diagnostic()?;
        ...
        let raw = adm2.raw_sealed().into_diagnostic()?;
        assert!(
