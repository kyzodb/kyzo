...
fn lift_tagged(obj: &Map<String, Value>) -> Option<DataValue> {
    ...
    match kind {
        "bytes" => {
            ...
            let Ok(bytes) = STANDARD.decode(s) else {
                return None;
            };
        ...
        "uuid" => {
            let s = obj.get("v")?.as_str()?;
            let Ok(u) = uuid::Uuid::try_parse(s) else {
                return None;
            };
        ...
        "regex" => {
            ...
            let Ok(flags_u8) = u8::try_from(flags_u) else {
                return None;
            };
            let flags = RegexFlags::from_bits(flags_u8)?;
            let Ok(src) = RegexSource::validated(flags, pattern) else {
                return None;
            };
...
mod tests {
    ...
    fn regex_flags_participate_in_json_identity() -> Result<()> {
        ...
        let cs = DataValue::Regex(
            RegexSource::validated(RegexFlags::NONE, "foo".into()).into_diagnostic()?,
        );
        assert_ne!(
            to_json(&ci),
            to_json(&cs),
    ...
    fn bytes_uuid_vector_round_trip_preserve_kind() -> Result<()> {
        ...
        let vec = DataValue::Vector(
            Vector::try_new(vec![1.0, -2.5, 0.0]).ok_or_else(|| miette!("try_new"))?,
        );
