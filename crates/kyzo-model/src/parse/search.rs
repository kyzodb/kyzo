...
fn build_infix(lhs: Result<FtsExpr>, op: Pair<'_>, rhs: Result<FtsExpr>) -> Result<FtsExpr> {
    ...
    Ok(match op.as_rule() {
        Rule::fts_and => match lhs {
            ...
            lhs @ FtsExpr::Literal(_) | lhs @ FtsExpr::Near(_) | lhs @ FtsExpr::Or(_) | lhs @ FtsExpr::Not(..) => {
        ...
        Rule::fts_or => match lhs {
            ...
            lhs @ FtsExpr::Literal(_) | lhs @ FtsExpr::Near(_) | lhs @ FtsExpr::And(_) | lhs @ FtsExpr::Not(..) => {
            ...
            for pair in pair.into_inner() {
                match pair.as_rule() {
                    Rule::pos_int => {
                        ...
                        distance = u32::try_from(i).map_err(|_| {
                            BadFtsNumber(pair.as_str().to_string(), span)
                        })?;
                    }
...
fn build_phrase(pair: Pair<'_>) -> Result<FtsLiteral> {
    ...
    FtsLiteral::new(core_text, is_quoted, booster.into()).ok_or_else(|| {
        miette::miette!(
            "invalid FTS literal: empty text must not carry a prefix or booster, and searchable terms require a positive finite booster"
        )
    })
}
