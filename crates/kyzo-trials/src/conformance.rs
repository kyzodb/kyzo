...
//!   the simulator's seeded fault/crash/power-cut controls to drive the SAME
//!   generic KV law as a workload under injected faults (sim-backed; compiles
//!   when a trials-visible sim door exists).
...
//!   byte-identical observations (sim-backed arm; same door).
//!
...
// ==================== sim-backed arms (DST + differential + sim proofs) ====================
//
// These arms need a trials-visible `SimStorage` / `TempTx` door. That door is
// not on the T1 allowlist (`conformance.rs` + `lib.rs` only). The source is
// preserved here and compiles when the cfg is armed after allowlist expand.
// `cfg(any())` keeps the tokens in-tree without type-checking unresolved
// `crate::storage::…` paths from the pre-peel seat.
