...
///
/// **P112 host door.** [`Script::Imperative`] parses and is a typed
/// refusal at execution (`runtime/db.rs` / `ImperativeNotWired`); query and
/// system genera run through `parse_script`. No module-level
/// `allow(dead_code)` on `parse` in `lib.rs` — Imperative AST unusedness
/// is a rustc warning, not a blanket hide.
#[derive(Debug)]
pub enum Script {
