...
mod tests {
    ...
    fn hnsw_create_insert_search<S: Storage>(db: Engine<S>) {
        ...
        let out = db
            ...
            .map_err(|e| miette!("hnsw search: {e}"))?;
        ...
        let d0 = out.rows()[0][0].get_float().ok_or_else(|| miette!("dist"))?;
        let d1 = out.rows()[1][0].get_float().ok_or_else(|| miette!("dist"))?;
