...
impl<S: Storage> Db<S> {
    ...
    fn run_sys_op(
        ...
        match op {
            // Integrity constraints (session/constraint.rs). Creation
                ...
                let rows = handle
                    ...
                    .map(|r| {
                        let kind = match &r.kind {
                            crate::session::catalog::IndexKind::Plain { .. } => "plain",
                            crate::session::catalog::IndexKind::Temporal => "temporal",
                            crate::session::catalog::IndexKind::Hnsw(..) => "hnsw",
                            crate::session::catalog::IndexKind::Fts(..) => "fts",
                            crate::session::catalog::IndexKind::Lsh { .. } => "lsh",
                        };
                        Tuple::from_vec(vec![
