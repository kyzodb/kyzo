...
fn test_predicates() -> Result<()>  {
    assert_eq!(
        op_is_null(&[DataValue::Null])?,
        DataValue::from(true)
    );
    assert_eq!(
        op_is_int(&[DataValue::from(1)])?,
        DataValue::from(true)
    );
    assert_eq!(
        op_is_int(&[DataValue::from(1.0)])?,
        DataValue::from(false)
    );
    assert_eq!(
        op_is_float(&[DataValue::from(1)])?,
        DataValue::from(false)
    );
    assert_eq!(
        op_is_float(&[DataValue::from(1.0)])?,
        DataValue::from(true)
    );
    assert_eq!(
        op_is_num(&[DataValue::from(1)])?,
        DataValue::from(true)
    );
    assert_eq!(
        op_is_num(&[DataValue::from(1.0)])?,
        DataValue::from(true)
    );
    assert_eq!(
        op_is_num(&[DataValue::Null])?,
        DataValue::from(false)
    );
    assert_eq!(
...
fn test_uuid() -> Result<()>  {
    ...
    assert!(op_is_uuid(&[v4])?.get_bool().ok_or_else(|| miette!("get_bool"))?);
