...
/// (`Expr::eval`) reads one `&[DataValue]` row at a time, and the batched
...
pub(crate) fn refine_batch(pred: &Option<Expr>, batch: Batch) -> Result<Batch> {
    ...
    let n_rows = rows.len();
    let columns = crate::exec::expr::batch::ColumnBatch::from_rows(rows, width)?;
    let sel = crate::exec::expr::eval::eval_pred_batched(
        pred,
        &columns,
        &crate::exec::expr::batch::Selection::all(n_rows)?,
    )?;
