...
fn compile(
    ...
    let dict = create_relation(&mut tx, input_handle("dict", meta)).unwrap();
    for (entity, surfaces) in rows {
        ...
        let key = dict.encode_key_for_store(&row, SourceSpan(0, 0)).unwrap();
        let val = dict.encode_val_for_store(&row, SourceSpan(0, 0)).unwrap();
