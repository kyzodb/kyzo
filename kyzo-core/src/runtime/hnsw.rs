...
mod tests {
    ...
    fn setup(
        ...
        let base = create_relation(&mut tx, input_handle("vecs", base_metadata())).unwrap();
    ...
    fn zero_vector_refused_typed_under_cosine() {
        ...
        let (base2, idx2, m2) = {
            ...
            let mut tx = db.write_tx().unwrap();
            let base2 = create_relation(&mut tx, input_handle("vecs2", base_metadata())).unwrap();
        ...
        let key = base2.encode_key_for_store(&zrow, SourceSpan(0, 0)).unwrap();
        let val = base2.encode_val_for_store(&zrow, SourceSpan(0, 0)).unwrap();
        tx.put(&key, &val).unwrap();
    ...
    fn re_put_detects_change_via_content_hash() {
        ...
        let mut tx = db.write_tx().unwrap();
        let key = base.encode_key_for_store(&moved, SourceSpan(0, 0)).unwrap();
        let val = base.encode_val_for_store(&moved, SourceSpan(0, 0)).unwrap();
        tx.put(&key, &val).unwrap();
