...
use crate::data::tuple::{Tuple, TupleT, extend_tuple_from_v};
...
use crate::runtime::relation::{
    AccessLevel, IndexKind, IndexRef, InsufficientAccessLevel, RelationHandle,
};
...
impl<T: WriteTx> SessionTx<T> {
    ...
    fn put_into_relation<S: Storage<WriteTx = T>>(
        ...
        for tuple in res_iter {
            ...
            if is_insert {
                // The read below is load-bearing under SSI: it conflict-
                // tracks the key, so a concurrent insert of the same key
                // aborts one of the racers (see storage/mod.rs — blind
                // writes are never validated).
                if self.exists_routed(relation_store.is_temp, &key)? {
            ...
            if need_to_collect || has_indices {
                if let Some(existing) = self.get_routed(relation_store.is_temp, &key)? {
                    ...
                    if has_indices && extracted != tup {
                        self.update_indices(relation_store, Some(&extracted), Some(&tup))?;
                    }
                    if need_to_collect {
                        old_tuples.push(tup);
                    }
                } else if has_indices {
                    self.update_indices(relation_store, Some(&extracted), None)?;
                }
    ...
    fn update_in_relation<S: Storage<WriteTx = T>>(
        ...
        for tuple in res_iter {
            ...
            let original_val: Tuple = match self.get_routed(relation_store.is_temp, &key)? {
                None => {
                    bail!(TransactAssertionFailure {
                        relation: relation_store.name.to_string(),
                        key: new_kv,
                ...
                Some(v) => {
                    // Fallible decode (the original unwrapped msgpack).
                    let mut old = Tuple::new();
                    extend_tuple_from_v(&mut old, &v)?;
                    old
                }
            };
            ...
            if need_to_collect || has_indices {
                if has_indices {
                    self.update_indices(relation_store, Some(&new_kv), Some(&old_kv))?;
                }
                if need_to_collect {
    ...
    fn remove_from_relation<S: Storage<WriteTx = T>>(
        ...
        for tuple in res_iter {
            let extracted: Tuple = key_extractors
                ...
                .try_collect()?;
            let key = relation_store.encode_key_for_store(&extracted, span)?;
            if check_exists && !self.exists_routed(relation_store.is_temp, &key)? {
                bail!(TransactAssertionFailure {
                    relation: relation_store.name.to_string(),
                    key: extracted,
            ...
            if need_to_collect || has_indices {
                if let Some(existing) = self.get_routed(relation_store.is_temp, &key)? {
                    ...
                    extend_tuple_from_v(&mut tup, &existing)?;
                    if has_indices {
                        self.update_indices(relation_store, None, Some(&tup))?;
                    }
                    if need_to_collect {
                        old_tuples.push(tup);
                    }
            ...
            self.del_routed(relation_store.is_temp, &key)?;
        }
    ...
    fn ensure_in_relation(
        ...
        for tuple in res_iter {
            ...
            match self.get_routed(relation_store.is_temp, &key)? {
                None => {
                    bail!(TransactAssertionFailure {
                        relation: relation_store.name.to_string(),
                        key: extracted,
            ...
            match &index.kind {
                IndexKind::Plain { mapper } => {
                    ...
                    if let Some(old) = old_kv {
                        let idx_tup: Tuple = project_mapper(mapper, old, relation_store)?;
                        let encoded =
                            idx_handle.encode_key_for_store(&idx_tup, SourceSpan::default())?;
                        self.del_routed(idx_handle.is_temp, &encoded)?;
                    }
                    if let Some(new) = new_kv {
                        let idx_tup: Tuple = project_mapper(mapper, new, relation_store)?;
                        let encoded =
                            idx_handle.encode_key_for_store(&idx_tup, SourceSpan::default())?;
                        // Index rows are key-only, as in the original.
                        self.put_routed(idx_handle.is_temp, &encoded, &[])?;
                    }
                }
...
pub(crate) enum IndexCtx {
    Hnsw {
        idx: RelationHandle,
        manifest: crate::runtime::hnsw::HnswIndexManifest,
        filter: Option<Vec<crate::data::expr::Bytecode>>,
    },
    Fts {
        idx: RelationHandle,
        extractor: Vec<crate::data::expr::Bytecode>,
        analyzer: Arc<crate::fts::tokenizer::TextAnalyzer>,
    },
    Lsh {
        idx: RelationHandle,
        inv: RelationHandle,
        ...
        extractor: Vec<crate::data::expr::Bytecode>,
        analyzer: Arc<crate::fts::tokenizer::TextAnalyzer>,
        perms: Arc<crate::runtime::minhash_lsh::HashPermutations>,
    },
}
...
impl<T: WriteTx> SessionTx<T> {
    ...
    pub(crate) fn apply_manifest_index(
        ...
        match ctx {
            IndexCtx::Hnsw {
                ...
                if let Some(old) = old_kv {
                    crate::runtime::hnsw::hnsw_remove(&mut self.store, base, idx, old)?;
                }
                if let Some(new) = new_kv {
                    crate::runtime::hnsw::hnsw_put(
                        &mut self.store,
                        manifest,
                        base,
                        idx,
            ...
            IndexCtx::Fts {
                ...
                if let Some(old) = old_kv {
                    crate::runtime::fts_index::fts_del(
                        &mut self.store,
                        old,
                        extractor,
                        &mut stack,
                ...
                if let Some(new) = new_kv {
                    crate::runtime::fts_index::fts_put(
                        &mut self.store,
                        new,
                        extractor,
                        &mut stack,
            ...
            IndexCtx::Lsh {
                ...
                if let Some(old) = old_kv {
                    crate::runtime::minhash_lsh::lsh_del(&mut self.store, old, None, idx, inv)?;
                }
                if let Some(new) = new_kv {
                    crate::runtime::minhash_lsh::lsh_put(
                        &mut self.store,
                        new,
                        extractor,
                        &mut stack,
    ...
    fn attach_and_backfill(
        ...
        for (name, metadata) in index_metas {
            self.create_relation(crate::data::program::InputRelationHandle {
        ...
        let ctx = self.manifest_index_ctx(&base, &index_ref)?;
        ...
        loop {
            let batch: Vec<Tuple> = self
                .store
                .range_scan_tuple(&lower, &upper)
                .take(BACKFILL_BATCH)
                .try_collect()?;
            let Some(last) = batch.last() else { break };
            lower = base
                .encode_key_for_store(last, SourceSpan::default())?
                .as_ref()
                .to_vec();
            lower.push(0);
            for row in &batch {
                self.apply_manifest_index(&base, &ctx, Some(row), None)?;
            }
        }
    ...
    pub(crate) fn create_hnsw_index(
        ...
        let manifest = crate::runtime::hnsw::HnswIndexManifest {
        ...
        let idx_meta = crate::runtime::hnsw::hnsw_index_metadata(&base.metadata);
    ...
    pub(crate) fn create_fts_index(
        ...
        let manifest = crate::fts::FtsIndexManifest {
            base_relation: cfg.base_relation.clone(),
            index_name: cfg.index_name.clone(),
        ...
        let idx_meta = crate::runtime::fts_index::fts_index_metadata(&base.metadata);
        let idx_ref = IndexRef {
            name: cfg.index_name.clone(),
    ...
    pub(crate) fn create_lsh_index(
        ...
        use crate::runtime::minhash_lsh::{
            DEFAULT_PERM_SEED, HashPermutations, LshParams, Weights,
        };
        let base = self.get_relation(&cfg.base_relation)?;
        cfg.tokenizer.build(&cfg.filters)?;
        Self::compile_row_expr(&base, &cfg.extractor)?;
        ...
        let manifest = crate::runtime::minhash_lsh::MinHashLshIndexManifest {
            base_relation: cfg.base_relation.clone(),
            index_name: cfg.index_name.clone(),
            extractor: cfg.extractor.clone(),
            n_gram: cfg.n_gram,
        ...
        let idx_meta = crate::runtime::minhash_lsh::lsh_index_metadata(&base.metadata);
        let inv_meta = crate::runtime::minhash_lsh::lsh_inv_index_metadata(&base.metadata);
        let idx_ref = IndexRef {
            name: cfg.index_name.clone(),
