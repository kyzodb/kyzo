...
2. Arm the path firewall:
   ```bash
   python3 /home/kyle/src/plan/scripts/kyzo_arm_session.py <allowlist-paths...>
...
Tail the child Task agent's tool stream (**tool name + path/command only**) — one short line per call. Never the agent's prose. Never wait for the summary.
