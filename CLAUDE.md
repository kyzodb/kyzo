...
Story pipeline (authority lives in tool grants):

1. **demolition** first — cuts the whole Condemned block; a red tree is by design (green is a `main` invariant), and shrinking the cut to stay green is the forbidden compromise. Skipping requires a stated, evidence-backed clean-surface finding.
2. **development-task** — one fresh agent per T#, spawned with the full contract pasted verbatim + a file allowlist. Its only exit is the judge's form.
...
Whip (SendMessage) on the FIRST tell — off-allowlist read/edit, forbidden target, self-reverts, spawning a sub-agent, no-op spin, idle with the box unchecked, board tools via ToolSearch, mechanism mismatch — never rehearsing its reasons. Repeat tell or anything destructive: `TaskStop`. Once the gate rules, stop — re-validation is your own fear-read.
...
Board MCP server: `mcp/` (gitignored); home = planner-dev repo (copy source to persist); reconnect MCP to load edits.
