...
> Cut (uncommitted relocate): whole `data/value/*` plane, `data/span`, `data/symb`, and `typestate` now live under `crates/kyzo-model/` (`value/`, `program/{span,symbol}`, `typestate.rs`). `data/json.rs` stays listed — NamedRows/diagnostic envelopes remain in core; DataValue↔JSON wire is `kyzo-model/src/envelope/json.rs`.
