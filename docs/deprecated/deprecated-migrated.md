...
## capacity.rs (37 lines; inventory: module doc (the one allocation-admission seam — every input-declared reservation admits here, bounded by a proven-finite `available`; the `allocation_admission` resonance check forbids per-site `.min(...)` caps on `with_capacity`/`reserve`), and `admit(declared, available)` — closed)
- **L1:** preserve-and-move whole → `session/capacity.rs` (seat exists:
  session owns the live admission path; capacity capping is a
  session-facing seam over proven-finite counts, not a value-plane
  fact). One concept at its natural size.
- **L2:** gold, preserve verbatim: the one-seam law and the unbounded
  case (when no finite `available` exists, do not reserve — grow
  amortized). Nothing to split.

## typestate.rs (38 lines; inventory: module doc (staged-construction idiom: zero-sized markers; required fields start `Unset`, setters flip to `Set`, `build()` exists only for the fully-`Set` instantiation; sealed product carries a private field so the builder is the sole constructor; compile-fail witnessed via `data::value::proofs::assert_not_impl`), `Unset`, `Set` — closed)
- **L1:** preserve-and-move whole → `crates/kyzo-model/typestate.rs`
  (seat exists: pure type-system vocabulary with no engine state —
  model crate owns it so every zone can share the idiom without
  importing session/exec). One concept at its natural size.
- **L2:** gold: compile-error-not-Option for missing required fields;
  monomorphization erasure of the markers; the sealed-product private
  field doctrine. Arrival: re-home the `assert_not_impl` witness cite
  beside the builders that use it once value proofs move.
...
## engines/projection.rs (325 lines; inventory: module doc (one generic build→seal→freshness machine; kind engines re-land as `K`; relation search via `RelationIndexSearch` not a ProjectionKind façade (P103); segment freshness consumes `Generation::classify`; meaning clocks from `runtime::generation`, cache cannot own truth (P106)), `ProjectionKind`, `SealedQuery` (+Query/Candidates/`search`), `RelationIndexSearch` (+Request/`search_relation`), `Generation` (`stamp_from_counter` / `raw` / `classify` / Display — no public `new(raw)`), `ResidentIndexKey` (+`for_relation`), `ProjectionBuilder<K>` (+new/kind_mut/kind/`seal`), `Sealed<K>` (+generation/kind/into_kind/ `query`), `Stale<K>` (+generation/expected/kind), and tests (builder_seal_query_machine, classify keep/mismatch, five engine kinds share one machine) — closed)
- **L1:** preserve-and-move whole → `project/projection.rs` (seat
  exists: the shared projection protocol every rebuildable index
  parameterizes). One concept — the build→seal→Stale machine — at its
  natural size; tests move with it. `Generation`'s mint door stays
  `session/generation.rs` (`CatalogGeneration::projection_stamp`); this
  file keeps the stamp type and classify surface.
- **L2:** gold: Stale-as-type not Option; no public `Generation::new`;
  RelationIndexSearch as the sole relation-backed search seam (P103);
  cache-never-owns-truth (P106). Nothing condemned.
...
## engines/text/README.md (5 lines; inventory: stopwords-iso source URL note for the vendored stop-word lists — closed)
- **L1:** preserve-and-move whole → `project/text/README.md` beside the
  tokenizer subtree it documents. One attribution note; no constructs.
- **L2:** preserve the upstream URL; nothing to reforge.
...
## runtime/generation.rs (99 lines; inventory: module doc (catalog freshness authorities — sole meaning-clock for sealed projections; never a bare `u64`, never public `Generation::new(raw)`), `CatalogGeneration` (+from_relation/from_index/`projection_stamp`), `RelationGeneration` (+witness/`projection_stamp`), `IndexGeneration` (+witness/`projection_stamp`) — closed)
- **L1:** preserve-and-move whole → `session/generation.rs` (seat
  exists: the catalog meaning-clock is session authority; it mints
  `project::projection::Generation` stamps and never lives under
  project/). One concept at its natural size.
- **L2:** gold: sole door `CatalogGeneration::projection_stamp`;
  RelationGeneration/IndexGeneration as witnesses that lift into the
  catalog clock; forbids bare AtomicU64 freshness. Arrival: keep the
  cross-crate import edge into `project/projection.rs`'s `Generation`.

## runtime/pinned_handle.hex (1 lines; inventory: pinned RelationHandle wire fixture consumed by `runtime/relation.rs` via `include_str!` — closed)
- **L1:** preserve-and-move whole → `session/pinned_handle.hex` beside
  the catalog/handle successor (`session/catalog.rs` / access) that
  includes it. Fixture bytes move with their consumer; no code
  constructs.
- **L2:** preserve byte-identical; the pin is a format-stability
  witness, not regenerable prose.
